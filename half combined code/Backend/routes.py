...
from .forms import CreateEventForm, BookingForm
...
# 3) Create Submission (POST) — Save image, store in database, set tickets_available=total_tickets (#4)
@bp.post("/events/new")
def event_create():
    form = CreateEventForm()
    ...
    if form.image.data:
        ...
        f.save(upload_root / unique)
        image_rel = f"uploads/{unique}"

    e = Event(
        title=form.title.data.strip(),
        description=form.description.data.strip(),
        cuisine_highlights=form.cuisine_highlights.data.strip(),
        date=form.date.data,
        time=form.time.data,
        venue=form.venue.data.strip(),
        total_tickets=form.total_tickets.data,
        tickets_available=form.total_tickets.data,  # Initial remaining tickets = Total tickets
...
# 4) Details Page (GET) — Display Remaining Tickets and Status (#9)
...
def event_detail(event_id: int):
    ...
    if not e:
        abort(404)
    form = BookingForm()
    return render_template("event_detail.html", event=e, form=form)

# 5) Reservation (POST) — Overbooking Prevention (#8) + Automatic Sold-Out (#9)
@bp.post("/events/<int:event_id>/book")
def event_book(event_id: int):
    e = db.session.get(Event, event_id)
    if not e:
        abort(404)
    form = BookingForm()
    ...
    if qty > e.tickets_available:
        flash("Requested tickets exceed available tickets.", "danger")  # #8
        return redirect(url_for("main.event_detail", event_id=event_id))

    e.tickets_available -= qty
    if e.tickets_available <= 0:
